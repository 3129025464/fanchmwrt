...
function index()
    ...
    entry({base[1], base[2], "qos"}, cbi("fwx/qos"), _("QoS"), 45)
    entry({base[1], base[2], "urlfilter"}, cbi("fwx/urlfilter"), _("URL Filter"), 47)
    entry({base[1], base[2], "ddos"}, cbi("fwx/ddos"), _("DDoS Protection"), 48)
    entry({base[1], base[2], "vpn"}, cbi("fwx/vpn"), _("VPN"), 49)
    entry({base[1], base[2], "alerts"}, template("fwx/alerts"), _("Alerts"), 50)
    entry({base[1], base[2], "vlan"}, cbi("fwx/vlan"), _("VLAN"), 51)
    entry({base[1], base[2], "ipsec"}, cbi("fwx/ipsec"), _("IPSec"), 52)
    entry({base[1], base[2], "audit"}, cbi("fwx/audit"), _("Audit Log"), 53)
    entry({base[1], base[2], "reports"}, cbi("fwx/reports"), _("Reports"), 54)
    entry({base[1], base[2], "rbac"}, cbi("fwx/rbac"), _("Access Control"), 55)
    entry({base[1], base[2], "sdwan"}, cbi("fwx/sdwan"), _("SD-WAN"), 56)
    entry({base[1], base[2], "ha"}, cbi("fwx/ha"), _("High Availability"), 57)
